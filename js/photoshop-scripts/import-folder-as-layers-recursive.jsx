...
function main() {
    ...
    getFilesRecursively(sourceFolder, prefs);
}

function getFilesRecursively(sourceFolder, prefs) {
    var arr = sourceFolder.getFiles();

    var fileArr = [];

    for (var i = 0; i < arr.length; i++) {
        // if it's a folder, recursively try to find files in it
        if (arr[i] instanceof Folder) {
            getFilesRecursively(arr[i], prefs);
        } else {
            // else add the file to file array
            fileArr.push(arr[i]);
        }
    }

    // filter file array to get only allowed extensions
    fileArr = getFiles(fileArr);
	
    // if files were found, proceed with import
    if (fileArr.length) {
		importFolderAsLayers(fileArr, prefs);
	}
}
